...
(Windows-hardened, for clients that spawn without a shell: `claude mcp add uia -- cmd /c bunx -y bun-uia`.) It exposes **37 snapshot-first tools** (protocol `2025-11-25`), gated by a deployer policy. `desktop_snapshot` returns a ref-keyed tree — `Button "Five" [ref=e49]` — then `click`/`invoke`/`type`/`set_value`/`toggle`/`scroll` target a ref (cursor-free, so they work on a minimized/background/occluded/locked window), and every action returns the **smallest faithful re-grounding** — a compact `Δ` delta when little changed (`~ Text "Display is 5" → "Display is 55"`, ~28× cheaper than a full dump), else a pruned, size-capped tree — so the model re-grounds without drowning in tokens. Beyond one window it can **see the whole desktop** (`screen_capture`), **see a specific occluded/GPU window** (`capture_window` — Windows.Graphics.Capture), turn a pixel into a control (`inspect_point`), read a control's full state (`inspect_element`), read a data grid / list / table cell-by-cell (`read_table`), read native/MSAA trees, list monitors, manage windows, and — gated default-OFF — launch apps, run programs, and read/write files. A thrown tool error comes back as `isError` so the loop self-corrects instead of stopping.
