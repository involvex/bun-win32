...
Listed in the official **[MCP Registry](https://registry.modelcontextprotocol.io/v0/servers?search=io.github.ObscuritySRL/bun-uia)** as `io.github.ObscuritySRL/bun-uia`. It exposes the whole surface as **60 policy-gated tools** (54 under the default `safe` profile; 22 under `readonly`) so an agent drives Windows cursor-free, sees the desktop, and (when enabled) launches apps and reads/writes files. Needs **Bun on `PATH`**.
...
- **MCP server for Claude** — `claude mcp add uia -- bunx bun-uia` exposes the whole surface as 60 policy-gated tools (54 under the default `safe` profile); the agent drives Windows cursor-free, sees the desktop, and (when enabled) launches apps and reads/writes files.
